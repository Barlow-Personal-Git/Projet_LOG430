...
async fn fetch_alerte_reapprovisionnements(base_url: &str) -> Result<Vec<AlerteReapprovisionnement>, reqwest::Error> {
    let client = Client::new();
...
async fn fetch_inventaires_surplus(base_url: &str) -> Result<Vec<InventairesSurplus>, reqwest::Error> {
    let client = Client::new();
...
async fn fetch_tendances_hebdomadaires(base_url: &str) -> Result<Vec<TendancesHebdo>, reqwest::Error> {
    let client = Client::new();
...
pub async fn performances() -> Result<Template, Status> {
    ...
    let alertes = fetch_alerte_reapprovisionnements(&base_url).await.map_err(|e| {
    ...
    let tendances = fetch_tendances_hebdomadaires(&base_url).await.map_err(|e| {
