...
use crate::dto::{ProduitVenduDTO, TransactionProduitsDTO};
use crate::models::transaction_produit::{
    NouveauTransactionProduit, SommeTransactionProduitParMagasin,
};
use crate::models::{Magasin, TransactionProduit};
use crate::schema::magasins::dsl::{id_magasin, magasins, nom};
use crate::schema::transaction_produits::dsl::{
    id_magasin as trp_magasin_id, id_transaction as trp_magasin_id_transaction,
    produits as trp_produits, total as trp_total, transaction_produits,
};
use diesel::dsl::sum;
use diesel::prelude::*;
...
pub async fn post_transaction_produits(
    data: Json<TransactionProduitsDTO>,
    ...
    info!(
        "Insertion des produits pour id_magasin {}",
        magasin_record.id_magasin
    );

    let new_trp: Vec<NouveauTransactionProduit> = data
        .transaction_produits
        .iter()
...
pub async fn get_ventes_magasin() -> Result<Json<Vec<SommeTransactionProduitParMagasin>>, String> {
    ...
    let sommes: Vec<SommeTransactionProduitParMagasin> = resultats
        .into_iter()
        .map(
            |(magasin_nom, magasin_total)| SommeTransactionProduitParMagasin {
                magasin: magasin_nom,
        ...
        .collect();
    info!(
        "Récupération réussie, {} transactions produits",
        sommes.len()
    );
...
pub async fn get_produits_vendus() -> Result<Json<Vec<ProduitVenduDTO>>, String> {
    ...
    let mut produits_vendus: Vec<ProduitVenduDTO> = compteur
        .into_iter()
        .map(|(nom_produit, nbr_vendue)| ProduitVenduDTO {
            nom_produit,
            nbr_vendue,
        })
        .collect();
    ...
    info!(
        "Récupération réussie, {} produits vendus",
        produits_vendus.len()
    );
